                            ...
                            <div className="flex flex-col lg:flex-row gap-6 justify-between items-start w-full">
                                ...
                                <div className="flex flex-col gap-2 shrink-0">
                                    ...
                                    <div className="flex gap-2.5">
                                        <button onClick={() => handleChange(card.id, 'gekozenRichting', card.gekozenRichting === 'A' ? '' : 'A')} className={`flex items-center gap-2 px-5 py-2.5 rounded-lg border-2 text-[11px] font-black uppercase tracking-widest transition-all ${card.gekozenRichting === 'A' ? 'bg-emerald-500 border-emerald-500 text-white shadow-md shadow-emerald-200' : 'bg-white border-slate-200 text-slate-500 hover:border-emerald-300 hover:text-emerald-600'}`}>
                                            {card.gekozenRichting === 'A' && <Check className="w-3.5 h-3.5" strokeWidth={4} />} Ontwerp A
                                        </button>
                                        <button onClick={() => handleChange(card.id, 'gekozenRichting', card.gekozenRichting === 'B' ? '' : 'B')} className={`flex items-center gap-2 px-5 py-2.5 rounded-lg border-2 text-[11px] font-black uppercase tracking-widest transition-all ${card.gekozenRichting === 'B' ? 'bg-violet-500 border-violet-500 text-white shadow-md shadow-violet-200' : 'bg-white border-slate-200 text-slate-500 hover:border-violet-300 hover:text-violet-600'}`}>
